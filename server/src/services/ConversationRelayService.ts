...
class ConversationRelayService implements ConversationRelay {
    ...
    /**
     * Generates a response from the AI based on a prompt.
     * Proxy method for direct access to response service functionality.
     * Used for triggering AI responses from external events (e.g., payment callbacks).
     *
     * @param {string} role - Message role ('user' or 'system')
     * @param {string} prompt - Prompt text to generate response from
     * @returns {Promise<void>} Resolves when response generation is complete
     */
    async generateResponse(role: 'user' | 'system', prompt: string): Promise<void> {
        await this.responseService.generateResponse(role, prompt);
    }

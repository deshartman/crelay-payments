...
- The `main` branch of this repository contains the Payments branch work from `simple-conversation-relay`
...
**Server Changes:**
- Removed direct dependency on AgentAssistedPaymentsService
- Payment functionality accessed directly through TwilioService methods
- Cleaner service initialization with no separate payment service

#### 📝 Files Modified

- `server/package.json` - Version bumped to 4.9.6
- `server/src/services/TwilioService.ts` - Consolidated payment methods, added configuration properties
- `server/src/services/AgentAssistedPaymentsService.ts` - Removed (consolidated into TwilioService)
- `server/.env.example` - Added CURRENCY and TOKEN_TYPE with documentation
- `CHANGELOG.md` - Added release v4.9.6 documentation
- `README.md` - Added Agent Assisted Payments (AAP) section with tools documentation
